...
pub fn main() {
    ...
    let mut has_operation: bool = false;

    if args.contains(&String::from("add")) || args.contains(&String::from("a")) {
        stage_changes(&repo);
        has_operation = true;
    }
    if args.contains(&String::from("commit")) || args.contains(&String::from("c")) {
        commit(&repo);
        has_operation = true;
    }
    if args.contains(&String::from("push")) || args.contains(&String::from("p")) {
        push(&repo);
        has_operation = true;
    }
...
fn choose_repo(repo: &Repository) -> String {
    ...
    print!("Which remote should be used? [1-{}]: ", i-1);
    io::stdout().flush().unwrap();
    ...
    io::stdin()
        ...
        .ok()
        .expect("ERR: Failed to read line.");
    remote_choice = String::from(remote_choice.trim());
    let remote_index_result = remote_choice.parse::<i32>();
    let remote_index = match remote_index_result {
        Ok(index) => index,
        Err(_e) => {
            println!("{}ERR: Input was not an integer{}", color::Fg(color::Red), color::Fg(color::White));
            process::exit(1);
        }
    };
